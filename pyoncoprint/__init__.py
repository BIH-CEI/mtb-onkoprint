...
def _get_text_bbox(t, ax, x=0, y=0, scale=[1, 1], fontdict=None):
    ...
    bb = t.get_window_extent(ax.figure.canvas.get_renderer()).transformed(
        ax.transData.inverted()).transformed(Affine2D().scale(*scale))
    if is_str:
        t.remove()
    return bb
...
class OncoPrint:
    ...
    def _sort_genes_default(self):
        ...
        sorted_indices = np.argsort(np.sum(cntmat, axis=1))[::-1]
    ...
    def oncoprint(self, markers, annotations={}, heatmaps={},
        ...
        if flag_annot:
            ...
            for i, (annot_type, annot_dic) in enumerate(sorted_annotations):
                ...
                if np.issubdtype(annots.dtype, np.number):
                    annot_min = 0
                    annot_gmax = np.max(annots)
        ...
        for mut in mutation_types:
            ...
            if mut in patch_mutations:
                ...
                pc_kwargs = {k: v for k, v in ms.items() if k not in ('marker', 'width', 'height', 'zindex',)}
        ...
        if legend:
            if legend_groups is not None:
                ax_legend = self._render_grouped_legend(
                    f, ax, ax_divider, ax_xlim, gap, background_lengths,
                    markers, legend_mut_to_patch, legend_mut_to_scatter,
                ...
                ax_legend = self._render_flat_legend(
                    f, ax, ax_divider, ax_xlim, gap, background_lengths,
                    markers, mutation_types, legend_mut_to_patch, legend_mut_to_scatter,
                    flag_annot, sorted_annotations if flag_annot else [],
                    heatmaps, rightplot, cell_background)
    ...
    def _render_flat_legend(self, f, ax, ax_divider, ax_xlim, gap, background_lengths,
                            markers, mutation_types, legend_mut_to_patch, legend_mut_to_scatter,
                            flag_annot, sorted_annotations, heatmaps, rightplot, cell_background):
        """Original flat legend renderer (fallback when legend_groups is not provided)."""
        ...
        def add_legend_item(label, col, cur_x, cur_y, is_first, is_mut):
            legend_item_width = background_lengths[0] + gap_handle_to_text + abs(
        ...
        def add_legend_scaler(min_val, max_val, col=None, cmap=None):
            ...
            if col is not None:
                ...
                ab = AnnotationBbox(imagebox,
                                    (cur_x + text_left_offset + scaler_left_pad,
                                     cur_y + background_lengths[1] * 0.5),
    ...
    def _render_grouped_legend(self, f, ax, ax_divider, ax_xlim, gap, background_lengths,
                               markers, legend_mut_to_patch, legend_mut_to_scatter,
        ...
        ax_scale = ax_size / ax_size_reduced
        bb = _get_text_bbox(" ", ax, x=0, y=0, scale=ax_scale,
                            fontdict={'size': legend_font_size})
        tw_space = abs(bb.width)
        text_height = abs(bb.height)
        swatch_size = legend_swatch_size
        line_height = max(text_height, swatch_size)
        text_left_offset = -bb.xmin
        text_top_offset = -bb.ymax

        plot_width = ax_xlim[1] - ax_xlim[0]
        col_width = plot_width / legend_columns
        pad_x = tw_space * 2
        pad_y = 0.2
        row_gap = line_height * 1.5
        group_internal_gap = line_height * 0.4
        ...
        ax_legend_height_ratio = ax_legend_height / (len(self.sorted_genes) - gap[1])
        ax_legend = ax_divider.append_axes("bottom",
                                           size="{0:.6%}".format(ax_legend_height_ratio), pad=1.5)
        ax_legend.set_xlim(ax_xlim)
        ax_legend.set_ylim([ax_legend_height, 0])
        ax_legend.tick_params(top=False, bottom=False, left=False, right=False,
        ...
        for item in legend_texts:
            if len(item) == 4:
                ...
                ax_legend.text(x, y, text, fontdict=fontdict)
            else:
                text, x, y = item
                ax_legend.text(x, y, text, fontdict={'size': legend_font_size})
        for (x, y), scatter_kwargs in legend_scatters:
            ax_legend.scatter([x], [y], **scatter_kwargs)
